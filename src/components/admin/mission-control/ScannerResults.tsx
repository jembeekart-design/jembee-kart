...
export default function ScannerResults() {
  ...
  const { healthScore, healthColor, healthLabel } = useMemo(() => {
    ...
    const color = score >= 95 ? "text-[var(--success-color)]" : score >= 80 ? "text-[var(--primary-color)]" : score >= 60 ? "text-yellow-600" : "text-[var(--danger-color)]";
      ...
      <div className="p-6 border-b bg-gray-50/30 flex justify-between">
        <div>
          ...
          <div className="grid grid-cols-4 gap-3 mt-4">
            ...
            <div className="rounded-lg bg-green-50 p-3 text-center"><p className="text-[10px] uppercase font-bold text-[var(--success-color)]">PASS</p><p className="text-xl font-black text-[var(--success-color)]">{summary.pass}</p></div>
            ...
            <div className="rounded-lg bg-red-50 p-3 text-center"><p className="text-[10px] uppercase font-bold text-[var(--danger-color)]">FAIL</p><p className="text-xl font-black text-[var(--danger-color)]">{summary.fail}</p></div>
          </div>
          <p className="mt-4 text-[10px] text-gray-400">Last Scan: {lastScan?.toLocaleTimeString() ?? "Pending"} | Scan Time: {scanTime}ms</p>
        </div>
        <button onClick={() => setLiveMode(!liveMode)} className={`px-2 py-1 h-6 rounded border text-[9px] font-bold uppercase ${liveMode ? "bg-green-100 text-[var(--success-color)]" : "bg-gray-100"}`}>Live : {liveMode ? "ON" : "OFF"}</button>
      </div>
        ...
        <div className="border-b bg-slate-50 p-5">
          ...
          <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="rounded-lg border bg-white p-3">
              ...
              <p className="font-bold text-[var(--success-color)]">Ready</p>
            </div>
            <div className="rounded-lg border bg-white p-3">
        ...
        {results.map((item) => (
          <div key={item.id} className="p-4 hover:bg-gray-50 transition-colors">
            <div className="flex justify-between items-start">
              ...
              <span className={`px-2 py-1 rounded text-[9px] font-bold uppercase flex items-center gap-1 ${item.status === "PASS" ? "bg-green-100 text-[var(--success-color)]" : item.status === "WARNING" ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-[var(--danger-color)]"}`}>
