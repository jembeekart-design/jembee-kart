// src/jembee-governance/scanners/securityScanner.ts
...
export class SecurityScanner {
  ...
  private readonly patterns = [
    {
      ...
      regex:
        /(api[_-]?key\s*[:=]\s*["'`][^"'`]{8,}["'`])|(AIza[0-9A-Za-z\-_]{20,})/gi,
...

    {
      ...
      regex:
        /(role\s*===?\s*["'`]admin["'`])|(isAdmin\s*=\s*true)|(admin\s*=\s*true)/gi,
      severity: "CRITICAL" as const,
    },

    {
      ...
      regex:
        /(secret|privateKey|clientSecret|jwtSecret|accessToken)\s*[:=]\s*["'`].+["'`]/gi,
      severity: "CRITICAL" as const,
    },
...

    {
      ...
      regex:
        /console\.(log|info|debug)\((.*password.*|.*secret.*|.*token.*|.*apikey.*)\)/gi,
  ...
  public scanProject(projectRoot: string): SecurityScanResult {
    ...
    for (const file of files) {
      ...
      if (
        result.some((v) => v.id === "SEC_API_KEY")
      ) {
        apiKeyExposed = true;
      }
    ...
    try {
      ...
      lines.forEach((line, index) => {
        for (const pattern of this.patterns) {
          ...
          violations.push({
            ...
            title: pattern.title,
            description:
              "Potential security risk detected during governance scan.",
            category: "SECURITY",
            ...
            recommendation:
              "Move sensitive values to environment variables and validate access controls.",
            detectedAt:
      ...
      if (
        ...
        violations.push({
          ...
          description:
            "Sensitive page appears to lack authentication validation.",
          category: "SECURITY",
          severity: "WARNING",
          filePath,
          recommendation:
            "Protect routes using authentication and role checks.",
          detectedAt:
            new Date().toISOString(),
        });
      }
