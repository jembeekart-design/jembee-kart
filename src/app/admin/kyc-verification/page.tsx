...
import { KYCService } from "@/firestore/services/KYCService";
...
export default function KYCVerificationPage() {
  ...
  async function updateStatus(id: string, status: string) {
    try {
      await kycService.updateKYCStatus(id, status);
    } catch (error) {
      console.error(error);
    ...
    <main className="min-h-screen bg-[var(--color-page-background)] p-4 text-[var(--button-text-color)]">
      ...
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--muted-text-color)]">Verify user identity documents</p>
        </div>
      </div>
              ...
              <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <div className="mt-4 flex flex-col gap-2 text-[var(--text-color)]">
                    ...
                    <div>
                      Document No: {request.documentNumber}
                    </div>
                  </div>
                </div>
...
function StatusBadge({ status }: { status: string }) {
  function getColor() {
    if (status === "Approved") return "bg-[var(--success-color)]";
    if (status === "Rejected") return "bg-[var(--danger-color)]";
    return "bg-[var(--warning-color)]";
  }

  function getIcon() {
    if (status === "Approved") return <ShieldCheck size={16} />;
    if (status === "Rejected") return <XCircle size={16} />;
    return <Clock3 size={16} />;
  }

  return (
    <div className={`flex items-center gap-2 rounded-full px-5 py-3 text-sm font-bold ${getColor()}`}>
      {getIcon()}
      {status}
    </div>
  );
