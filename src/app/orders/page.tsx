...
export default function MyOrdersPage() {
  ...
  const getStatusColor = (s: string) => ({
    'pending': 'text-[var(--warning-color)] bg-[var(--warning-color)] border-[var(--warning-color)]', 
    'placed': 'text-[var(--primary-color)] bg-[var(--primary-color)] theme-primary-border',
    'shipped': 'text-[var(--primary-color)] bg-[var(--primary-color)] border-[var(--primary-color)]', 
    'delivered': 'text-[var(--success-color)] bg-[var(--success-color)] border-[var(--success-color)]',
    'cancelled': 'text-[var(--danger-color)] bg-[var(--danger-color)] border-[var(--danger-color)]'
  ...
  return (
    <main className="min-h-screen bg-[var(--primary-color)] pb-24">
      ...
      <div className="bg-gradient-to-br from-[var(--primary-color)] to-[var(--primary-color)] text-[var(--button-text-color)] p-6 pb-28 rounded-b-[2.5rem] shadow-xl">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[ {l: "Total Orders", v: stats.total}, {l: "Total Spent", v: `₹${stats.spent}`}, {l: "Delivered", v: stats.delivered}, {l: "Pending", v: stats.pending} ].map((s, i) => (
             <div key={i} className="bg-[var(--card-color)]/10 p-3 rounded-2xl border border-[var(--border-color)]/10"><p className="text-[9px] opacity-70">{s.l}</p><p className="font-black text-sm">{s.v}</p></div>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 -mt-12 mb-6">
        <div className="flex gap-2 overflow-x-auto no-scrollbar bg-[var(--card-color)] p-1.5 rounded-2xl shadow-sm border border-[var(--border-color)]">
          {['All Orders', 'Pending', 'Placed', 'Shipped', 'Delivered', 'Cancelled'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 rounded-xl text-[10px] font-black whitespace-nowrap transition-all ${activeTab === tab ? 'bg-[var(--primary-color)] text-[var(--button-text-color)] shadow-lg' : 'bg-transparent text-[var(--muted-text-color)]'}`}>
      ...
      <section className="px-4 space-y-4">
        {filteredOrders.length === 0 ? (
          <div className="text-center py-20 bg-[var(--card-color)] rounded-3xl shadow-sm"><Box size={50} className="mx-auto text-[var(--text-color)]"/><p className="font-black mt-4">No Orders Found</p></div>
          ...
          <div key={order.id} className="bg-[var(--card-color)] p-5 rounded-3xl shadow-sm border border-[var(--border-color)]">
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-1 cursor-pointer" onClick={() => { navigator.clipboard.writeText(order.orderNumber || order.id); showToast("Copied!"); }}>
                    ...
                    <Copy size={10} className="text-[var(--primary-color)]" />
                </div>
                <p className="text-xl font-black text-[var(--primary-color)] mt-1">₹{order.finalAmount || 0}</p>
            ...
            <div className="flex justify-between items-center mt-4 pt-4 border-t border-[var(--border-color)]">
              ...
              <Link href={`/orders/${order.id}`} className="text-xs font-black text-[var(--primary-color)] underline">View Details →</Link>
            </div>
          </div>
        ))}
      </section>
      ...
      <div className="fixed bottom-0 w-full bg-[var(--card-color)] border-t border-[var(--border-color)] px-6 py-4 flex justify-between items-center z-50">
        {[ {icon: Home, l: "Home", p: "/"}, {icon: LayoutGrid, l: "Categories", p: "/categories"}, {icon: ShoppingBag, l: "Cart", p: "/cart"}, {icon: ListOrdered, l: "Orders", p: "/orders"}, {icon: User, l: "Profile", p: "/profile"} ].map((n, i) => (
          <Link href={n.p} key={i} className="flex flex-col items-center gap-1">
            <n.icon size={20} className={pathname === n.p ? "text-[var(--primary-color)]" : "text-[var(--muted-text-color)]"} />
            <span className={`text-[9px] font-bold ${pathname === n.p ? "text-[var(--primary-color)]" : "text-[var(--muted-text-color)]"}`}>{n.l}</span>
          </Link>
        ))}
      </div>
    </main>
