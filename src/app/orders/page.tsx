...
export default function MyOrdersPage() {
  ...
  const getStatusColor = (s: string) => ({
    ...
    'placed': 'text-blue-500 bg-blue-50 theme-primary-border',
  ...
  }[s?.toLowerCase()] || 'text-[var(--muted-text-color)] bg-gray-50 border-gray-100');

  return (
    <main className="min-h-screen bg-[#f8f9fe] pb-24">
      {toast && <div className="fixed top-5 left-1/2 -translate-x-1/2 z-[100] bg-black text-[var(--button-text-color)] px-6 py-3 rounded-full flex items-center gap-2 text-xs font-bold shadow-xl"><Check size={14}/> {toast}</div>}
      ...
      <div className="px-4 -mt-12 mb-6">
        <div className="flex gap-2 overflow-x-auto no-scrollbar bg-[var(--card-color)] p-1.5 rounded-2xl shadow-sm border border-gray-100">
      ...
      <section className="px-4 space-y-4">
        {filteredOrders.length === 0 ? (
          ...
          <div key={order.id} className="bg-[var(--card-color)] p-5 rounded-3xl shadow-sm border border-gray-100">
      ...
      <div className="fixed bottom-0 w-full bg-[var(--card-color)] border-t border-gray-100 px-6 py-4 flex justify-between items-center z-50">
